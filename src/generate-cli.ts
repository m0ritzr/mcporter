...
async function readPackageMetadata(): Promise<{
	...
	try {
		...
		return {
			name: typeof parsed.name === "string" ? parsed.name : "mcp-runtime",
			version: typeof parsed.version === "string" ? parsed.version : "unknown",
		};
	} catch {
		return { name: "mcp-runtime", version: "unknown" };
	}
}
...
function renderTemplate({
	...
	const imports = [
		...
		"import { createCallResult } from 'mcp-runtime';",
	].join("\n");
	...
	const generatorHeader = `Generated by ${generator.name}@${generator.version} — https://github.com/steipete/mcp-runtime`;
...
program.option('-c, --config <path>', 'Alternate mcp-runtime.json path to load server definition.');
