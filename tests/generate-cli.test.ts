...
describe("generateCli", () => {
	it("creates a standalone CLI and bundled executable", async () => {
		...
		const generatorLabel = `${packageJson.name ?? "mcp-runtime"}@${packageJson.version ?? "unknown"}`;
		expect(tsContent).toContain(
			`Generated by ${packageJson.name ?? "mcp-runtime"}@${packageJson.version ?? "unknown"} — https://github.com/steipete/mcp-runtime`,
		);
